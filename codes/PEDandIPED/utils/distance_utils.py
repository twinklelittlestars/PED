...
def build_all_distance_lookups(data_instance, sorted_thresholds, num_processes):
    ...
    for attribute in attributes:
        ...
        # print(f"Unique values length for {attribute}: {len(unique_values)}")
...
def compute_distance_chunk(chunk, unique_block, unique_full, attribute):
    ...
    for i, j in chunk:
        vi = unique_block[i]
        vj = unique_full[j]
        if pd.isnull(vi) or pd.isnull(vj):
            continue
        if vi == vj:
            dist = 0
        else:
...
def build_all_distance_lookups_for_block(
    ...
    for attribute in attributes:
        ...
        if max_threshold == 0:
            # print(f"Skipping {attribute} since max threshold is 0.")
            continue
        block_values = preprocess_values([row[attribute] for row in block_data])
        full_values = preprocess_values([row[attribute] for row in full_data])
        unique_block = list(set(block_values))
        ...
        distance_lookups[attribute] = merged
