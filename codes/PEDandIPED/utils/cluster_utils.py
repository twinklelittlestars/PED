...
def compute_clusters_for_chunk_partial(row_indices, col_indices):
        ...
        for attr in g_attributes:
            ...
            for j in col_indices:
                ...
                if max_threshold == 0:
                    ...
                    dist = g_distance_lookups[attr].get((val_i, val_j), g_distance_lookups[attr].get((val_j, val_i), float("inf")))
                    if dist <= max_threshold:
                        cluster_l.append(j)
...
def generate_partial_clusters(row_indices, data_instance, sorted_thresholds, distance_lookups, num_processes, col_indices):
    attributes = list(sorted_thresholds.keys())
    row_splits = [[] for _ in range(num_processes)]
    for idx, row_id in enumerate(row_indices):
        row_splits[idx % num_processes].append(row_id)
    ...
    with multiprocessing.Pool(
        ...
        results = pool.starmap(compute_clusters_for_chunk_partial, [(split, col_indices) for split in row_splits])
    print("Generate partial clusters based on the completed results")
    ...
    for attr in attributes:
            ...
            for i, cluster_j_list in prev_clusters:
                ...
                for j in cluster_j_list:
                    ...
                    if t == 0:
                        ...
                        dist = distance_lookups[attr].get((val_i, val_j), distance_lookups[attr].get((val_j, val_i), float("inf")))
                        if dist <= t:
                            filtered_j_list.append(j)
        ...
        clusters[attr] = attribute_clusters
        print("Completed clustering for attribute", attr)
